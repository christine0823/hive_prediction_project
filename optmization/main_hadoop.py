...
result = gbrt_minimize(f, bound, n_calls=20, n_random_starts=10, acq_func='EI', acq_optimizer='auto', x0=result.x, y0=result.fun, random_state=None, verbose=True, callback=None, n_points=10000, xi=0.01, kappa=1.96, n_jobs=-1)
...
def generate_output(x):
	dag_output = open(output_file+'_all', "w")
	for i in range(len(x)):
		value = ""	
		if i == len(x)-1:
			value = "-Xmx"+str(x[i])+"m"
